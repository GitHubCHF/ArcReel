...
class OSSUploader:
    ...
    def __init__(self, config: OSSConfig, *, sign_expires_seconds: int = DEFAULT_SIGN_EXPIRES_SECONDS) -> None:
        ...
        endpoint = config.endpoint if "://" in config.endpoint else f"https://{config.endpoint}"
        auth = oss2.Auth(config.access_key_id, config.access_key_secret)
        self._bucket = oss2.Bucket(auth, endpoint, config.bucket)
